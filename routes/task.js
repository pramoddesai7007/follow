...
router.get('/taskCounts', jwtMiddleware, async (req, res) => {
  try {
    ...
    const pendingTasks = await Task.countDocuments({ assignTo: userId, status: 'pending', deadlineDate: { $gt: currentDate } }); // Adjusted query for pending tasks with deadline less than or equal to current date
...
router.get('/adminTaskCounts', jwtMiddleware, async (req, res) => {
  try {
    ...
    const pendingTasks = await Task.countDocuments({ assignedBy: adminId, status: 'pending', deadlineDate: { $gt: currentDate } }); // Adjusted query for pending tasks with deadline less than or equal to current date
...
//         $match: {
//           assignedBy: { $exists: true },
//           startDate: { $lte: sevenDaysAgo }, // Changed condition to $gte (greater than or equal)
//         },
//       },
//       {
//         $group: {
...
//sms api for reminder



// async function sendSMS(name, phoneNumber, amount, status) {
